...
namespace mvc_blog_projesi.Controllers
{
    ...
    {
        // GET: Author
        BlogManager blogmanage=new BlogManager(new EfBlogDal());
        ...
        {
            var authordetail=blogmanage.GetBlogByID(id);
        ...
        {
            var blogauthorid = blogmanage.GetList().Where(x => x.BlogID == id).Select(y => y.AuthorID).FirstOrDefault();

            var authorblogs = blogmanage.GetBlogByAuthor(blogauthorid);
